 ...
 * - One-off (cadence null): flat one-off rate × bins.
 * - Seasonal: $105 covers the SEASON (3 washes), so each wash bills 1/3 — the
 *   customer pays $105/yr total across 3 visits. × bins.
 * - Other recurring: per-clean rate × bins.
 */
export function baseChargeCents(cadence: Cadence | null, binCount: number): number {
  const bins = Math.max(1, binCount || 1);
  if (cadence === null) return ONEOFF_PRICE_CENTS * bins;
  if (cadence === 'seasonal') return Math.round((CADENCE_PRICE_CENTS.seasonal / 3) * bins);
  return CADENCE_PRICE_CENTS[cadence] * bins;
}
